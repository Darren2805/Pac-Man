...
Ghost* ghost_create(int flag) {
	...
	// TODO-GC-ghost udah: Create other type ghost, load corresponding sprites.
...
Ghost* akatsuki_create(int flag) {
	...
	ghost->flee_sprite = load_bitmap("Assets/Akatsuki_flee.png");
	ghost->dead_sprite = load_bitmap("Assets/Akatsuki_dead.png");
	...
	case Blinky:
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		ghost->move_sprite = load_bitmap("Assets/Ghost_itachi_brown.png");
		ghost->move_script = &ghost_move_script_random;
		break;
	case Pinky:
		// *load move script of shortest_path
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		ghost->move_sprite = load_bitmap("Assets/Ghost_itachi_pink.png");
		ghost->move_script = &ghost_move_script_random;
		break;
	case Inky:
		// *load move script of shortest_path
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		ghost->move_sprite = load_bitmap("Assets/Ghost_itachi_green.png");
		ghost->move_script = &ghost_move_script_random;
		break;
	case Clyde:
		// *load move script of shortest_path
		ghost->objData.Coord.x = cage_grid_x;
		ghost->objData.Coord.y = cage_grid_y;
		ghost->move_sprite = load_bitmap("Assets/Ghost_itachi.png");
		ghost->move_script = &ghost_move_script_shortest_path;
		break;
	default:
		ghost->objData.Coord.x = cage_grid_x;
...
void ghost_destory(Ghost* ghost) {
	// TODO-GC-memory udah..: free ghost resource
...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE) {
		// TODO-PB-animation udah: ghost FLEE animation, draw blue flee sprites,
		...
		*/ 
		float time = (float)get_power_up_timer_tick();
		float duration = (float)get_power_up_duration();
			if (time > (duration*60)/100)
			{
				...
				al_draw_scaled_bitmap(ghost->flee_sprite, 0 + bitmap_x_offset + offset, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
			...
			{
				if ((ghost->objData.moveCD / 2) % 32 >= 0 && (ghost->objData.moveCD / 2) % 32 <= 15) {
					offset = 0;
				}
				else if ((ghost->objData.moveCD / 2) % 32 >= 16 && (ghost->objData.moveCD / 2) % 32 <= 31) {
					offset = 16;
				}
		...
		// TODO-PB-animation udah: ghost going animation
		...
		// TODO-GC-animation udah: ghost animation
		// *draw ghost->move_sprite
			...
			{
			case LEFT:
				al_draw_scaled_bitmap(ghost->move_sprite, offset + 32, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
			...
			case RIGHT:
				...
				break;
			case UP:
				al_draw_scaled_bitmap(ghost->move_sprite, offset + 64, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
				break;
			case DOWN:
				al_draw_scaled_bitmap(ghost->move_sprite, offset + 96, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
...
bool ghost_movable(const Ghost* ghost, const Map* M, Directions targetDirec, bool room) {
	...
	int index_x = ghost->objData.Coord.x; int index_y = ghost->objData.Coord.y;
...
void ghost_toggle_FLEE(Ghost* ghost, bool setFLEE) {
	// TODO-PB udah: change ghosts state when power bean is eaten by pacman.
