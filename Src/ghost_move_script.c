...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	// TODO-GC-random_movement udah: (Not in Hackathon) 
	// Description:
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	// TODO-PB udah: escape from pacman
	// Description:
	...
	Directions counter_one = shortestDirection;
	static Directions proba[4]; // possible movement
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if (i != shortestDirection && ghost_movable(ghost, M, i, ghost->status == FLEE)) 	proba[cnt++] = i;
	if (cnt >= 1) {
		ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)]);
	}
	else { // for the dead end case
...
void ghost_move_script_shortest_path(Ghost* ghost, Map* M, Pacman* pacman) {
	// TODO-GC-movement udah: do a little modification on shortest path move script
	...
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			break;
		case FREEDOM:
			if (ghost->away_from_pacman_x < 3 || ghost->away_from_pacman_y < 3)
			{
