...
#define GHOST_NUM 4 
// TODO-GC-ghost udah: create a least FOUR ghost!
/* global variables*/
extern const uint32_t GAME_TICK_CD;
extern uint32_t GAME_TICK;
extern ALLEGRO_TIMER* game_tick_timer;
int game_value_win = 0;
int game_main_Score = 0;
bool game_over = false;
bool game_win = false;
bool wall_detector = false;
...
static void init(void) {
	...
	// TODO-GC-read_txt udah: Create map from .txt file so that you can design your own map!!
	basic_map = create_map("Assets/map_darren.txt");
	if (!basic_map) {
	...
	else if(pacmanCounter == 2)
	{
		pman = naruto_create();
	}
	else if (pacmanCounter == 3)
	{
		pman = kakashi_create();
	}
	if (!pman) {
...
static void checkItem(void) {
	...
	case '.':
		...
		break;
	case 'P':
		// TODO-GC-PB udah: ease power bean
...
static void status_update(void) {
	// TODO-PB udah: check powerUp duration
	//game_log("%d\n", ghosts[0]->status);
	if (pman->powerUp)
	{
		int duration = al_get_timer_count(power_up_timer);
		if (duration < power_up_duration)
		{
			for (int index = 0; index < GHOST_NUM; index++)
			{
				ghost_toggle_FLEE(ghosts[index], true);
			}
		}
		else
		{
			for (int index = 0; index < GHOST_NUM ; index++)
			{
				ghost_toggle_FLEE(ghosts[index], false);
			}
		}
	...
	for (int i = 0; i < GHOST_NUM; i++) {
		RecArea pacman = getDrawArea((object*)pman, GAME_TICK_CD);
		RecArea ghost = getDrawArea((object*)ghosts[i], GAME_TICK_CD);
		int index_x = (pman->objData.Coord.x) - (ghosts[i]->objData.Coord.x);
		if (index_x < 0)
		{
			index_x *= -1;
		}
		int index_y = (pman->objData.Coord.y) - (ghosts[i]->objData.Coord.y);
		if (index_y < 0)
		{
			index_y *= -1;
		}
		ghosts[i]->away_from_pacman_x = index_x;
		ghosts[i]->away_from_pacman_y = index_y;
		...
		{
			// TODO-GC-game_over udah: use `getDrawArea(..., GAME_TICK_CD)` and `RecAreaOverlap(..., GAME_TICK_CD)` functions to detect if pacman and ghosts collide with each other.
			...
			if(!cheat_mode && RecAreaOverlap(&pacman,&ghost))
			{
		...
		{
			// TODO-GC-PB udah: if ghost is collided by pacman, it should go back to the cage immediately and come out after a period.
			RecArea pacman = getDrawArea((object*)pman, GAME_TICK_CD);
			RecArea ghost = getDrawArea((object*)ghosts[i], GAME_TICK_CD);
...
static void update(void) {
	...
	if (game_over) {
		// TODO-GC-game_over udah: start pman->death_anim_counter and schedule a game-over event (e.g change scene to menu) after Pacman's death animation finished
		// hint: refer al_get_timer_started(...), al_get_timer_count(...), al_stop_timer(...), al_rest(...)
			...
			game_value_win = 0;
			game_main_Score = 0;
			al_get_timer_started(pman->death_anim_counter);
			int time = al_get_timer_count(pman->death_anim_counter)*16;
			if (time == 14*16)
			{
				al_rest(2.0);
...
static void draw(void) {
	...
	// TODO-GC-scoring udah: Draw scoreboard, something your may need is sprinf();
	char arr[1001];
	snprintf(arr, sizeof(arr), "Score : %d", game_main_Score);
	al_draw_text(
		menuFont,
...
static void destroy(void) {
	// TODO-GC-memory udah: free map array, Pacman and ghosts
	pacman_destroy(pman);
	delete_map(basic_map);
	for (int index = 0; index < GHOST_NUM; index++)
	{
		ghost_destory(ghosts[index]);
	}
}
...
static void on_key_down(int key_code) {
	switch (key_code)
	{
		...
		case ALLEGRO_KEY_S:
			...
			if (cheat_mode && key_state[ALLEGRO_KEY_LCTRL])
			{
				for (int index = 0;index < GHOST_NUM;index++)
				{
					if(ghosts[index]->speed == 2)
					{
						ghosts[index]->speed = 0;
					}
					else if(ghosts[index]->speed == 0)
					{
						ghosts[index]->speed = 2;
					}
				}
			}
			break;
		...
		case ALLEGRO_KEY_K:
			if (cheat_mode)
			{
				for (int index = 0; index < GHOST_NUM; index++)
				{
					ghosts[index]->status = GO_IN;
					ghosts[index]->speed = 4;
				}
			}
			break;
