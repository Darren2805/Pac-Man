...
static ALLEGRO_BITMAP* GAME_LATARBELAKANG = NULL;
static ALLEGRO_SAMPLE_ID menuBGM;
static int gameTitleW ;
static int gameTitleH ;
static int GAME_LATARBELAKANG_LEBAR;
static int GAME_LATARBELAKANG_TINGGI;
...
static void init() {
	...
	if (pacmanCounter >= 2)
	{
		gameTitle = load_bitmap("Assets/Naruto_title_3.png");
		gameTitleW = al_get_bitmap_width(gameTitle);
		gameTitleH = al_get_bitmap_height(gameTitle);
		GAME_LATARBELAKANG = load_bitmap("Assets/gambar_background_2.jpg");
		GAME_LATARBELAKANG_LEBAR = al_get_bitmap_width(GAME_LATARBELAKANG);
		GAME_LATARBELAKANG_TINGGI = al_get_bitmap_height(GAME_LATARBELAKANG);
	}
	else if(pacmanCounter == 1 && (checkCounter == true))
	{
		gameTitle = load_bitmap("Assets/title.png");
		gameTitleW = al_get_bitmap_width(gameTitle);
		gameTitleH = al_get_bitmap_height(gameTitle);
		GAME_LATARBELAKANG = load_bitmap("Assets/black.jpg");
		GAME_LATARBELAKANG_LEBAR = al_get_bitmap_width(GAME_LATARBELAKANG);
		GAME_LATARBELAKANG_TINGGI = al_get_bitmap_height(GAME_LATARBELAKANG);
	}
	else
...
static void draw() {
	...
	al_draw_scaled_bitmap(
		GAME_LATARBELAKANG,
		0, 0,
		GAME_LATARBELAKANG_LEBAR, GAME_LATARBELAKANG_TINGGI,
		0, 0,
		SCREEN_W, SCREEN_H,
		0
	);
...
static void destroy() {
	...
	al_destroy_bitmap(GAME_LATARBELAKANG);
