...
Map* create_map(const char * filepath) {
	...
	if (filepath == NULL) {
		...
		// TODO-GC-read_txt udah: use fopen to open a file stream
	...
	*/
	...
	for (int i = 0; i < M->row_num; i++) {
		for (int j = 0; j < M->col_num; j++) {
			if (filepath == NULL)
				...
				// TODO-GC-read_txt udah: input the map from file to M->map[row][col] 
				// '#' -> wall
				// '.' -> beans
				// 'B' -> room of ghost
				// 'P' -> Power Bean 
	...
	//game_log("ASKDJASDJKAJSKDJAKSJD\n");
...
void delete_map(Map* M) {
	if (!M)
		return;
	// TODO-GC-memory udah: free the dynamic allocated part of Map* M at here;
	
	if (M->map)
	{
		for (int index = 0; index < M->row_num; index++)
			free(M->map[index]);
	}
	free(M);
}
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				case '#':
					...
					break;
				// TODO-PB udah: draw the power bean
...
static void draw_block_index(const Map* M, const int row, const int col) {
	...
	else if (pacmanCounter == 3)
	{
		...
		if (!(U && UR && UL && B && BR && BL && R && L)) {
			...
			al_draw_filled_rectangle(s_x, s_y,
				e_x, e_y, al_map_rgb(128, 128, 128));
			if (row < M->row_num - 1 && B && !(BL && BR && R && L)) {
				s_x = block_x + dw;
				s_y = block_y + dw;
				e_x = s_x + dw;
