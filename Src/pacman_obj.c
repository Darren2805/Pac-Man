...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	int index_x = pacman->objData.Coord.x; int index_y = pacman->objData.Coord.y;
	...
	if (!wall_detector && is_wall_block(M, index_x , index_y) || is_room_block(M, index_x , index_y))
		return false;
...
Pacman* pacman_create() {
	...
	pman->win_anim_counter = al_create_timer(1.0f / 8.0f);
	pman->powerUp = false;
	/* load sprites */
...
Pacman* naruto_create() {
	...
	pman->powerUp = false;
	/* load sprites */
	pman->move_sprite = load_bitmap("Assets/naruto_move.png");
	pman->die_sprite = load_bitmap("Assets/Naruto_die.png");
	return pman;
}

Pacman* kakashi_create() {
	// Allocate dynamic memory for pman pointer;
	Pacman* pman = (Pacman*)malloc(sizeof(Pacman));
	if (!pman)
		return NULL;
	...
	pman->powerUp = false;
	/* load sprites */
	pman->move_sprite = load_bitmap("Assets/Kakashi_move.png");
	pman->die_sprite = load_bitmap("Assets/Kakashi_die.png");
	return pman;
}

void pacman_destroy(Pacman* pman) {	
	// TODO-GC-memory udah: free pacman resource
...
void pacman_draw(Pacman* pman) {
	// TODO-GC-animation udah: Draw Pacman and animations
	...
	int offset = 0;
	if (!game_over) {
		// TODO-GC-animation udah : We have two frames for each direction. You can use the value of pman->objData.moveCD to determine which frame of the animation to draw.
		...
		if ((pman->objData.moveCD / 2) % 32 >= 0 && (pman->objData.moveCD / 2) % 32 <= 15) {
			offset = 0;
		}
		else if ((pman->objData.moveCD / 2) % 32 >= 16 && (pman->objData.moveCD / 2) % 32 <= 31) {
			offset = 16;
		}
		...
		*/
		...
		{
		case LEFT:
			al_draw_scaled_bitmap(pman->move_sprite, offset + 32, 0,
				16, 16,
		...
		case RIGHT:
			...
			break;
		case UP:
			al_draw_scaled_bitmap(pman->move_sprite, offset + 64, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
			);
			break;
		case DOWN:
			al_draw_scaled_bitmap(pman->move_sprite, offset + 96, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
			);
		...
		// TODO-GC-animation udah: Draw die animation(pman->die_sprite)
...
void pacman_die() {
	// TODO-GC-game_over udah: play sound of pacman's death! see shared.c
